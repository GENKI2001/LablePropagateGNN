...
def main(dataset_name='Cornell', model_name='RobustH2GCN', calc_neighbor_label_features=True, 
    ...
    # NUM_LAYERSは [1,2,3] に固定(RobustH2GCNの場合は 1 に固定)
    # MAX_HOPSは [2,3,4]
    ...
    def save_experiment_results(all_results, total_combinations, DATASET_NAME, MODEL_NAME, 
                            ...
                            FEATURE_MODIFICATIONS, USE_EDGE_MODIFICATION, EDGE_MODIFICATIONS,
                            USE_SIMILARITY_BASED_EDGES, USE_EARLY_STOPPING,
        ...
        # EDGE_MODIFICATIONSが設定されている場合、ディレクトリ名に改変情報を追加
        if USE_EDGE_MODIFICATION and EDGE_MODIFICATIONS:
            for i, mod in enumerate(EDGE_MODIFICATIONS):
                mod_type = mod.get('type', 'unknown')
                percentage = mod.get('percentage', 0.0)
                modification_suffix += f"-edge-{mod_type}-{percentage:.1f}"
        
        if modification_suffix:
        ...
        experiment_config = {
            ...
            "feature_modifications": FEATURE_MODIFICATIONS,
            "use_edge_modification": USE_EDGE_MODIFICATION,
            "edge_modifications": EDGE_MODIFICATIONS,
    ...
    # 実験前にエッジ改変を適用
    if USE_EDGE_MODIFICATION:
        print(f"\n=== 実験前エッジ改変適用 ===")
        print(f"改変設定数: {len(EDGE_MODIFICATIONS)}")
        
        data.edge_index, edge_modification_info = apply_edge_modifications(data.edge_index, data.num_nodes, EDGE_MODIFICATIONS, device)
        
        # 改変情報を表示
        print_edge_modification_info(edge_modification_info, DATASET_NAME)
        print(f"改変適用後のエッジ数: {data.edge_index.shape[1]}")
    else:
        edge_modification_info = {
            'original_edges': data.edge_index.size(1),
            'final_edges': data.edge_index.size(1),
            'modifications_applied': [],
    ...
    print(f"エッジ改変使用: {USE_EDGE_MODIFICATION}")
    if USE_EDGE_MODIFICATION:
        print(f"改変設定数: {len(EDGE_MODIFICATIONS)}")
        for i, mod in enumerate(EDGE_MODIFICATIONS):
            mod_type = mod.get('type', 'unknown')
            percentage = mod.get('percentage', 0.0)
            print(f"  改変 {i+1}: エッジ{mod_type} - 割合: {percentage:.1%}")
    ...
    if total_combinations > 1:
        ...
        for i, param_combination in enumerate(param_combinations):
            ...
            for run in range(NUM_RUNS):
                ...
                if USE_EDGE_MODIFICATION:
                    run_result['edge_modification_info'] = edge_modification_info
        ...
        for i, result in enumerate(all_results):
            ...
            if USE_FEATURE_MODIFICATION and 'modification_info' in result and result['modification_info'] is not None:
                ...
                modification_info = f", 特徴量改変={mod_count}個"
            
            edge_modification_info = ""
            if USE_EDGE_MODIFICATION and 'edge_modification_info' in result and result['edge_modification_info'] is not None:
                edge_mod_count = len(result['edge_modification_info'].get('modifications_applied', []))
                edge_modification_info = f", エッジ改変={edge_mod_count}個"
            ...
            print(f"実験 {i+1:2d}: Final Test={result['final_test_acc']:.4f}, Best Test={result['best_test_acc']:.4f}{alpha_info}{modification_info}{edge_modification_info}{early_stop_info}{grid_search_info}")
    ...
    # 単一パラメータ実行（Grid Searchを使用しない場合）
        ...
        for i, result in enumerate(all_results):
            ...
            print(f"実験 {i+1:2d}: Final Test={result['final_test_acc']:.4f}, Best Test={result['best_test_acc']:.4f}{alpha_info}{modification_info}{edge_modification_info}{early_stop_info}")
    ...
    # 改変情報の統計表示
    if USE_FEATURE_MODIFICATION:
        print(f"特徴量改変設定: {len(FEATURE_MODIFICATIONS)}個の改変")
        for i, mod in enumerate(FEATURE_MODIFICATIONS):
            mod_type = mod.get('type', 'unknown')
            percentage = mod.get('percentage', 0.0)
            if mod_type == 'noise':
    ...
    if USE_EDGE_MODIFICATION:
        print(f"エッジ改変設定: {len(EDGE_MODIFICATIONS)}個の改変")
        for i, mod in enumerate(EDGE_MODIFICATIONS):
            mod_type = mod.get('type', 'unknown')
            percentage = mod.get('percentage', 0.0)
            print(f"  改変 {i+1}: エッジ{mod_type} - 割合: {percentage:.1%}")
