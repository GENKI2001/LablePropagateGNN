...
def create_combined_features_with_pca(data, device, max_hops=2, exclude_test_labels=True, 
                                    ...
                                    use_deepwalk=False, walk_length=10, num_walks=10, window_size=5):
    """
    PCAで次元圧縮した元の特徴量と隣接ノードのラベル特徴量を結合する関数（DeepWalkオプション付き）
    ...
    Args:
        ...
        use_deepwalk (bool): DeepWalk特徴量を使用するか（デフォルト: False）
        walk_length (int): 各ランダムウォークの長さ（デフォルト: 10）
        num_walks (int): 各ノードから開始するウォーク数（デフォルト: 10）
        window_size (int): スキップグラムのウィンドウサイズ（デフォルト: 5）
    ...
    Returns:
        ...
        deepwalk_features: DeepWalkで収集された特徴量（use_deepwalk=Trueの場合）
    """
    ...
    if use_deepwalk:
        print(f"DeepWalk設定: walk_length={walk_length}, num_walks={num_walks}, window_size={window_size}")
    ...
    # DeepWalk特徴量を作成（オプション）
    deepwalk_features = None
    if use_deepwalk:
        # 隣接関係を構築
        edge_index = data.edge_index.cpu()
        num_nodes = data.num_nodes
        ...
        # DeepWalkでラベル特徴量を収集
        deepwalk_features = torch.zeros((num_nodes, one_hot_labels.shape[1]), dtype=torch.float32)
        ...
        def random_walk(start_node, walk_length):
            ...
            for _ in range(walk_length - 1):
                if current_node in neighbors_dict and neighbors_dict[current_node]:
                    current_node = random.choice(neighbors_dict[current_node])
        ...
        def collect_labels_from_walk(walk, window_size):
            """ウォークからラベル特徴量を収集"""
            collected_features = []
            ...
            for i, center_node in enumerate(walk):
                ...
                end_idx = min(len(walk), i + window_size // 2 + 1)
                
                window_nodes = walk[start_idx:end_idx]
                
                if exclude_test_labels:
                    # 訓練ノードのみのラベルを使用
                    train_window_nodes = [n for n in window_nodes if data.train_mask[n]]
                    if train_window_nodes:
                        window_labels = one_hot_labels[train_window_nodes]
                        avg_label = window_labels.mean(axis=0)
                    else:
                        avg_label = np.zeros(one_hot_labels.shape[1])
                else:
                    # 全てのノードのラベルを使用
                    window_labels = one_hot_labels[window_nodes]
                    avg_label = window_labels.mean(axis=0)
                
                collected_features.append(avg_label)
        ...
        for start_node in range(num_nodes):
            ...
            for _ in range(num_walks):
                # ランダムウォークを実行
                walk = random_walk(start_node, walk_length)
                
                if len(walk) > 1:  # 少なくとも2ノード以上
                    # ウォークからラベル特徴量を収集
                    walk_features = collect_labels_from_walk(walk, window_size)
                    node_features.extend(walk_features)
            
            if node_features:
                # ノードの全ウォークの特徴量を平均
                node_features_array = np.array(node_features)
                deepwalk_features[start_node] = torch.tensor(node_features_array.mean(axis=0), dtype=torch.float32)
        
        print(f"DeepWalk特徴量の形状: {deepwalk_features.shape}")
    ...
    if use_deepwalk and deepwalk_features is not None:
        # PCA + DeepWalk + 隣接ノード特徴量を結合
        combined_features = torch.cat([pca_tensor, deepwalk_features, neighbor_features], dim=1)
        
        print(f"結合後の特徴量の形状: {combined_features.shape}")
        print(f"  - PCA特徴量: {pca_tensor.shape[1]}次元")
        print(f"  - DeepWalk特徴量: {deepwalk_features.shape[1]}次元")
    ...
    if use_deepwalk:
        return data, adj_matrix, one_hot_labels, pca_features, deepwalk_features
    else:
...
def create_deepwalk_label_features(data, device, walk_length=10, num_walks=10, window_size=5, 
                                  max_hops=2, exclude_test_labels=True, pca_components=50, 
                                  original_features=None):
    """
    DeepWalkでラベルのワンホットベクトルを収集した特徴量を結合する関数
    ...
    Returns:
        ...
        adj_matrix: 隣接行列
        one_hot_labels: ワンホットエンコーディングされたラベル
        deepwalk_features: DeepWalkで収集された特徴量
    """
    ...
    # 全ての特徴量を結合: PCA + DeepWalk + 隣接ノード特徴量
    pca_tensor = torch.tensor(pca_features, dtype=torch.float32)
    neighbor_features = data_with_neighbor_features.x
    ...
    return data, adj_matrix, one_hot_labels, deepwalk_features

def create_deepwalk_only_features(data, device, walk_length=10, num_walks=10, window_size=5, 
                                 exclude_test_labels=True):
    """
    DeepWalkでラベルのワンホットベクトルを収集した特徴量のみを作成する関数
    ...
    # 特徴量を設定
    data.x = deepwalk_features.to(device)
    
    return data, deepwalk_features 